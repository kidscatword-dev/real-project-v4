...
X_BOUNDARIES = [0, 410, 819, 1229, 1638, 2048]
Y_BOUNDARIES = [0, 410, 819, 1229, 1638, 2048]
PADDING = 5
...
def main() -> None:
    if len(sys.argv) != 4:
        raise SystemExit("Usage: split_m21_equipment_grid.py INPUT_IMAGE CELLS_JSON OUTPUT_DIRECTORY")
    source = Path(sys.argv[1])
    cells = json.loads(Path(sys.argv[2]).read_text(encoding="utf-8"))
    output_dir = Path(sys.argv[3])
    if len(cells) != 18 or len({item["term"] for item in cells}) != 18:
        raise RuntimeError("Expected 18 unique non-empty M21 equipment cells")

    image = Image.open(source).convert("RGBA")
    if image.size != (2048, 2048):
        raise RuntimeError(f"Expected a 2048×2048 M21 grid, found {image.size}")
    ...
    for index, cell in enumerate(cells, start=1):
        ...
        filename = f"m21-equipment-{index:02d}.png"
        destination = output_dir / filename
        cropped = image.crop(crop)
        non_white_pixels = count_non_white_pixels(cropped)
        if non_white_pixels < 3_000:
