...
X_BOUNDARIES = [5, 208, 409, 612, 813, 1019]
Y_BOUNDARIES = [2, 214, 423, 630, 838, 1024]
PADDING = 4
...
def main() -> None:
    if len(sys.argv) != 4:
        raise SystemExit("Usage: split_m26_profession_grid.py INPUT_IMAGE CELLS_JSON OUTPUT_DIRECTORY")
    source = Path(sys.argv[1])
    cells = json.loads(Path(sys.argv[2]).read_text(encoding="utf-8"))
    output_dir = Path(sys.argv[3])
    if len(cells) != 18 or len({item["term"] for item in cells}) != 18:
        raise RuntimeError("Expected 18 unique non-empty M26 profession cells")

    image = Image.open(source).convert("RGBA")
    if image.size != (1024, 1024):
        raise RuntimeError(f"Expected a 1024×1024 M26 grid, found {image.size}")
    ...
    for index, cell in enumerate(cells, start=1):
        ...
        filename = f"m26-profession-{index:02d}.png"
